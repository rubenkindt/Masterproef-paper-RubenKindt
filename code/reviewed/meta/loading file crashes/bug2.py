    ...
    for root, dirs, files in os.walk(path):
        for file in files:
            if file.__contains__("."):
                continue
            if not file.__contains__("prob026_sport_scheduling16650537264701257"):
                continue
...
def recordCrash(mmodel, executionDir, seedFolder, seedName, solver, trace=None, errorName=None):
    ...
    unloadableModel = "constraints: " + "\n"
    unloadableModel += str(mmodel.modifModel.constraints) + "\n" + "\n" + "\n"

    unloadableModel += "general model: " + "\n"
    unloadableModel += str(mmodel.modifModel)
    create_file(unloadableModel, os.path.join(path_to_bug_dir, "crash_info.txt"))
...
def satMutation(metaModel):
    ...
    metamorphicMutations= ['uselessAll', 'cons->cons2', 'sementicFusion!=', 'sementicFusion+']
