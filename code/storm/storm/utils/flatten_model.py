...
def flatten_cpmpymodel(cpmpyObject):
    ...
    """
    from cpmpy.model import Model  # otherwise circular dependency...
    orig_model = cpmpyObject.model
    # the top-level constraints
    basecons = []
    for con in orig_model.constraints:
        cpmpyObject.append_to_all_nodes(con)
        basecons += flatten_constraint(cpmpyObject, con)
    ...
    if orig_model.objective_ is None:
        ...
        if orig_model.objective_is_min:
            return cpmpyObject, Model(*basecons, minimize=newobj)
        else:
            return cpmpyObject, Model(*basecons, maximize=newobj)


def flatten_constraint(cpmpyObject, expr):
    """
    ...
    if isinstance(expr, bool):
        if expr:
            ...
            return cpmpyObject, [expr]  # not sure about this one... means False is a valid FNF expression
    elif isinstance(expr, _BoolVarImpl):
        return cpmpyObject, [expr]
    ...
    if is_any_list(expr):
        ...
        for e in expr:
            ...
            cpmpyObject, fc = flatten_constraint(cpmpyObject, e)  # add all at end
            flatcons += fc
