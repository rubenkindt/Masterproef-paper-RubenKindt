...
To illustrate that both MiniZinc and CPMpy are not that far apart we included a CPMpy listing (listing \ref{lst:SendMoreMoneyCPMpy}) for the same problem as we have seen for MiniZinc.


\label{lst:SendMoreMoneyCPMpy}
\begin{lstlisting}[float=t, language=python, caption={Solution to the puzzle "send more money". Modified from the CPMpy repository \url{https://github.com/CPMpy/cpmpy/blob/master/examples/send_more_money.py}}]
#!/usr/bin/python3
"""
...
model = Model() # with "+=" we add a constraint to the model
