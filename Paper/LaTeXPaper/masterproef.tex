...
\begin{abstract}
This thesis presents a comparative study between three ways of finding bugs in CPMpy as a use case to examine which techniques are suitable to find bugs in constraint programming languages. With these languages 
logical, optimization and other problems can be solved, which CPMpy extends with modeling of those problems in python. Here we will search for bugs where CPMpy may crash, hang or produce the wrong output. All of these are unwanted but the last ones do not show the user that a bug has occurred. 
The first technique to find bugs builds further on an existing paper to test SMT theories, which this paper converts to be able to test CPMpy with. 
...
None of the techniques found every single bug, with the first one finding 10 bugs, the second 13 and the last one finding 11 out of the 19 found bugs. Resulting in no clear winner. With more development in constraint programming and CPMpy these techniques can help with finding future bugs. 
